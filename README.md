...
Our method, LoRA-SB, leverages the same architecture. We find that updating *R* using its gradients *g^R* is equivalent to updating the full-finetuning matrix *W* with an equivalent gradient *g̃_{SB} = sBg^RA*. 
