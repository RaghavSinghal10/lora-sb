...
Our method, LoRA-SB, leverages the same architecture. We find that updating *R* using its gradients *g^R* is equivalent to updating the full-finetuning matrix *W* with an equivalent gradient *g̃\_SB = sBg^RA*. 

We initialize *B*, *R*, and *A* such that the equivalent gradient *g̃\_SB* optimally approximates the full fine-tuning gradient *g* in low rank subspaces **at each training step**.
